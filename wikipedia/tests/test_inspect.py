...
def _summary(tmp_path: Path) -> inspect.BundleSummary:
...
def test_inspect_bundle_accepts_deleted_shards_and_counts_checkpoint(tmp_path: Path) -> None:
    paths = BundlePaths.from_dir(tmp_path)
...
def test_main_prints_bundle_database_latency_and_chunks(
    ...
    monkeypatch.setattr(inspect, "Encoder", lambda bundle_dir: encoder)
