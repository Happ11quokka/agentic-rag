...
# Fig 9 variant: enable longest-common-prefix KV cache reuse across requests.
# --cache-reuse N tells llama-server to keep up to N tokens of the prior
# prompt's KV cache and reuse them when the next prompt shares that prefix.
exec "$LLAMA_BIN" \
    -m "$MODEL" \
