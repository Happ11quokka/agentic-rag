...
#### 재시작은 로컬 인덱스 93 GB를 통째로 버린다 — 이 구성에서 가장 비싼 함정

컨테이너가 죽었을 때 `volumes/milvus/data`에 93 GB가 남아 있는 것을 보고 "다시 받을
필요는 없겠다"고 판단했다. **틀렸다.** 스택을 올리자마자 Milvus가 지운다.

```
[roles/roles.go:107] ["Clean local data cache"] [path=/var/lib/milvus/data/mmap]
[roles/roles.go:107] ["Clean local data cache"] [path=/var/lib/milvus/data/indexnode]

재시작 직후: 93 GB → 6.2 GB
```

Milvus는 로컬 스토리지를 **자기가 소유한 캐시**로 보고 기동 때 비운다. 그래서
DiskANN 인덱스는 매번 MinIO에서 다시 내려받는다 — 그리고 MinIO도 **같은 HDD**에 있으므로,
한 스핀들에서 93 GB를 읽고 93 GB를 쓰는 일이 재시작마다 반복된다.

| | 세그먼트당 로드 |
|---|---|
| 앞선 시도의 잔여 파일이 로컬에 있던 구간 | 약 2분 |
| 콜드(MinIO에서 다시 받는) 구간 | **7~9분** |

> **따라서 이 컬렉션이 로드된 상태에서는 Milvus를 재시작하면 안 된다.** 컨테이너를
> 다시 만드는 모든 행위(compose 값 변경, Docker Desktop 재시작, VM 크기 변경)가
> 5시간짜리 재적재를 부른다. 튜닝이 필요하면 [etcd 런타임 설정](#튜닝은-재시작-없이--etcd-런타임-설정)을
> 쓰고, 재시작이 불가피하면 **그 전에 바꿀 것을 모두 모아서 한 번에** 바꾼다.
>
> VM 크기를 28 → 22 GB로 줄인 것도 이 대가를 치렀다. 다만 그건 치를 수밖에 없었다 —
> 28 GB에서는 검색 단계가 반드시 다시 죽는다.
...
### 결과 — 10M 검색 지연 (2026-08-05)

로드 완료 후 서로 다른 질의 10개.

```
latency_ms: [386042, 296698, 330993, 299184, 290625, 293654, 255080, 265980, 328232, 271727]
summary: cold=386,042 ms  mean=301,822 ms  median=295,176 ms  min=255,080 ms  max=386,042 ms
```

| | 1M 부분집합 (08-03) | **10M (08-05)** |
|---|---|---|
| 콜드 1회차 | 17.7 s | **386 s (6.4분)** |
| 이후 질의 | 71–80 ms | **255–331 s (4.3–5.5분)** |
| 워밍업 효과 | 있음 (캐시 적중) | **없음** |

**이 표의 마지막 줄이 핵심이다.** 1M에서는 2회차부터 71–80 ms로 떨어졌는데, 그건 검색이
빨라진 게 아니라 4 GB짜리 데이터가 램에 통째로 들어가 페이지 캐시를 재고 있었기
때문이다(19번에서 같은 질의 반복 문제를 고친 뒤에도 규모 문제는 남아 있었다).

10M에서는 10회차(272 s)가 2회차(297 s)보다 빠르지 않다. 로컬 인덱스 **93 GB**가
VM 22 GB를 압도해 캐시가 들어설 자리가 없다. 검색 내내 disk6은 **78 tps / 약 25 KB/t**,
즉 랜덤 읽기 IOPS 상한에 붙어 있었다(4절에서 잰 68 IOPS와 같은 벽).

> **7절이 예고한 "1M은 최종 구성이 아니다"가 여기서 정산됐다.** 10M은 워킹셋이 캐시를
> 넘어서므로 HDD 처치가 실제로 살아 있다. 1M의 웜 수치는 폐기가 맞았다.

#### 그런데 이 값은 prefetch에게 나쁜 소식이다

5.1절의 상한을 그대로 적용하면:

| retrieval | decode (8B 모델이 Thought 하나 생성) | 숨김 상한 | 회수율 |
|---|---|---|---|
| **295 s** | 약 2–5 s | 2–5 s | **0.7–1.7%** |

**즉 지금 구성에서 prefetch로 회수할 수 있는 비율은 2% 미만이다.** 5.1절이
*"검색이 decode보다 압도적으로 느리면 회수율은 0에 수렴한다 — 이건 강한 처치가 아니라
고장"* 이라고 적어 둔 바로 그 구간에 들어와 있다. 47M/Qdrant의 "48분에도 미완료"에서
"5분"으로 내려왔지만, **의미 있는 구간(`retrieval ≈ decode`, 초 단위)에는 아직 못 왔다.**

따라서 이어서 도는 쌍대 실험은 decode 간격을 **측정된 중앙값(295 s)에 맞춘다.** 이건
현실적인 에이전트를 흉내내는 게 아니라, *decode가 retrieval만큼 길다면 기구가 실제로
숨기는가*를 재는 것이다 — 기구의 상한 검증이지 운영 조건의 재현이 아니다. 임의의 값
(예: 2 s)을 넣으면 "1.7% 회수"라는, prefetch가 아니라 그 임의값에 대한 산수를 보고하게 된다.

**다음 단계는 규모가 아니라 매체·구성이다.** `retrieval`을 초 단위로 내리려면
(a) SSD 대조군, (b) `search_list` 축소(recall 대가), (c) 코퍼스 축소(처치 약화) 중
하나를 택해야 하고, 셋 다 트레이드오프가 명시적이다.

### 한계 — Docker Desktop 파일공유가 이 부하를 못 견딘다 (2026-08-05)

지연 측정은 끝났는데, **이어서 돌린 prefetch 실험 도중 Docker 엔진이 또 죽었다.** 이번엔
메모리가 아니었다(호스트 여유 83%). Docker 로그에 원인이 있다.

```
"error":"service fs failed: injecting event blocked for 60s"
```

`fs`는 `/Volumes/agentic_rag/...`를 VM 안으로 넣어 주는 **파일공유(virtiofs) 서비스**다.
이 서비스가 60초 동안 블로킹되면 워치독이 서비스를 죽이고, 엔진이 같이 내려간다.
검색이 USB HDD를 IOPS 상한(78 tps)으로 계속 때리면 이 60초 문턱을 넘는다.
**이 로그에만 6번 찍혔다.**

| | |
|---|---|
| 원인 | Milvus도 메모리도 아님 — Docker Desktop의 bind mount 계층 |
| 위치 | 검색 경로 **전체**가 이 계층을 통과한다 (인덱스가 bind mount 위에 있으므로) |
| 대가 | 죽을 때마다 로컬 인덱스가 날아가 **5시간 재적재** |

Docker Desktop은 USB 외장 HDD로 가는 bind mount에 지속적인 랜덤 I/O를 태우는 용도를
버티지 못한다. **이건 이 실험 플랫폼의 구조적 제약이지 설정 실수가 아니다.**

#### 처치 — 캐시 비율을 Milvus 기본값으로 되돌린다

`COMMON_DISKINDEX_SEARCHCACHEBUDGETGBRATIO`를 **0.01 → 0.10**(Milvus 기본값)으로 올렸다.
근거는 두 가지다.

1. **원래 논거가 그렇다.** 6절에서 Qdrant binary 양자화를 기각한 이유가
   *"실제로 운용되는 구성이 아니다"* 였다. 캐시를 기본값의 1/10로 조인 0.01도 같은
   기준에 걸린다 — 아무도 그렇게 안 쓴다. 0.10이 오히려 이 실험의 원칙에 맞는다.
2. **워치독을 넘기지 않으려면 디스크 큐를 낮춰야 한다.** 캐시가 늘면 디스크로 가는
   읽기가 줄어든다.

**저장매체는 그대로 HDD다.** 바뀐 것은 "실제 배포가 인덱스에 허용하는 만큼의 캐시를
준다"는 점뿐이다. 다만 **0.01에서 잰 수치(콜드 386 s / 중앙값 295 s)와 이후 수치는
서로 다른 구성이므로 비교하면 안 된다.**
