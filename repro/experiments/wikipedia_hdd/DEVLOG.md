...
| 19 | 08-03 | `benchmark_query`가 같은 질의를 10회 반복 → 2회차부터 페이지 캐시를 측정 | **미해결** — 디스크 기반 인덱스에서는 부적절한 측정 방식. 서로 다른 질의 N개로 바꿔야 한다. 웜 수치(71–80 ms)는 하한으로만 읽을 것 |
...
3. **`benchmark_query`가 같은 질의를 10회 반복한다.** 같은 질의는 그래프에서 같은 경로를 밟으므로 2회차부터는 페이지 캐시를 재는 것이지 검색을 재는 것이 아니다. **디스크 기반 인덱스에서는 이 측정 방식 자체가 부적절하다.** 서로 다른 질의 N개로 바꿔야 한다 — 다음 작업.

따라서 신뢰할 수 있는 것은 **콜드 값(로드 25.5분, 첫 검색 17.7초)** 이고, 웜 값은 상한이 아니라 하한으로 읽어야 한다.
