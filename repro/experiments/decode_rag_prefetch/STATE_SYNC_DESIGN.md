...
캐시는 **예측 문자열이 실제 질의와 정확히 같을 때만** 쓴다. 불일치면 기존 경로를 그대로 탄다.

`normalize()`는 캐시 **키**에만 쓴다. 대소문자·공백만 다른 예측이 각자 검색을 시작하는
낭비를 막기 위해서다. 그러나 **serve 시점의 판정은 원문 동일성**이어야 한다 — BGE-M3는
대소문자와 공백에 민감해서 `"Aurora  Borealis"`와 `"aurora borealis"`는 다른 벡터가 되고,
따라서 다른 passage가 나올 수 있다. 키만 보고 내주면 **정확도 불변이 조용히 깨진다.**
